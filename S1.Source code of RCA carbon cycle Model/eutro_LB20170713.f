...
C        2 VSNET1  NET SETTLING RATE FROM WATER COLUMN             M/DAY
C                  TO THE BED FOR ALGAL GROUP NO. 1
C        3 VSNET2  NET SETTLING RATE FROM WATER COLUMN             M/DAY
C                  TO THE BED FOR ALGAL GROUP NO. 2
C        4 VSNET3  NET SETTLING RATE FROM WATER COLUMN             M/DAY
C                  TO THE BED FOR ALGAL GROUP NO. 3
C        5 VSNET4  NET SETTLING RATE FROM WATER COLUMN             M/DAY
...
C    LB 20170325
      REAL
     .      TALK(NX,NY,NZ)  , CDTALK(NX,NY,NZ)
     .    , HIC(NX,NY,NZ)   , CDHIC(NX,NY,NZ)
     .    , pCO2(NX,NY,NZ)  , CDpCO2(NX,NY,NZ)
     .    , DIC(NX,NY,NZ)   , CDDIC(NX,NY,NZ)
     .    , SR290(NX,NY,NZ) , CO2SOL(NX,NY,NZ)
     .    , SR29PA1(NX,NY,NZ) , SR29PB1(NX,NY,NZ)
      REAL
     .      SR22PA(NX,NY,NZ) , SK1820A(NX,NY,NZ)
     .    , SK1921A(NX,NY,NZ) 
      REAL
     ...
     .    ,KAOPT,KEOPT,KECONST,KETVF,KADPO4,KADSI
      REAL
     .     SR29PA,SR29PB,ff,K1,K2,Kb,Kw,borate,Htotal,X,SDIC1
     .    ,TDIC1,X0,SchmidtN,X_lo,X_hi,X_mid,ftest,K12
      REAL
     .     fni(3)  
      REAL     EXPVALS(2000)
C    LB 20170325      
...
C        LABELED COMMON FOR EUTROPHICATION ALGAL GROWTH ROUTINES
     ...
     .   ,K1RBT,K1GRZCT,K2RBT,K2GRZCT,K3RBT,K3GRZCT
...
C             SEDIMENT SUBMODELS
     ...
     .  ,(CFLUXS(1,1,9),JDIC1(1,1))   , (CFLUXS(1,1,10),JTALK1(1,1))
     .  ,(CFLUXS(1,1,11),JPOCN(1,1))
...
C        LABELED COMMON FOR SEGMENT KES
     ...
     . ,(CARAY(1,1,1,27),DIC(1,1,1))  ,(CDARAY(1,1,1,27),CDDIC(1,1,1))
C     . ,(CARAY(1,1,1,28),pCO2(1,1,1))  ,(CDARAY(1,1,1,28),CDpCO2(1,1,1))
C     . ,(CARAY(1,1,1,28),HIC(1,1,1))  ,(CDARAY(1,1,1,28),CDHIC(1,1,1))
     . ,(CARAY(1,1,1,28),TALK(1,1,1))  ,(CDARAY(1,1,1,28),CDTALK(1,1,1))
...
C        COMPUTE CO2 TRANSFER COEFFICIENT BYLB 20170325
...
CHJT        IF(ITEMP.LE.10.) K1415CT(ITEMP)=0.
CHJT        IF(ITEMP.GT.10. .AND.ITEMP.LT.140.)
CHJT     .     K1415CT(ITEMP)= (FLOAT(ITEMP-10)/130.)*K1415CT(ITEMP)     !added by Jiatang Hu
CHJT        IF(ITEMP.LE.100.) K1415CT(ITEMP)=0.
CHJT        IF(ITEMP.GT.100. .AND.ITEMP.LT.140.)
CHJT     .     K1415CT(ITEMP)= (FLOAT(ITEMP-100)/40.)*K1415CT(ITEMP)     !corrected by Jiatang Hu
CHJT     .     K1415CT(ITEMP)= (FLOAT(140-ITEMP)/40.)*K1415CT(ITEMP)
      ...
      DO 60 IZ=1,NZ
       DO 60 IY=1,NY
        DO 60 IX=1,NX
         ...
         TLDOC = PHYT1(IX,IY,IZ) + PHYT2(IX,IY,IZ) +PHYT3(IX,IY,IZ) +
...
c$& share(rpoc_dda,lpoc_dda,rdoc_dda,ldoc_dda,repoc_dda,redoc_dda)        
...
c$& share(chla1_dmin,chla2_dmin,chla3_dmin)
     ...
     .          1000.*PHYT3(IX,IY,IZ)/(CCHL3S(IX,IY,IZ)+1.0E-4))
   84   CONTINUE
...
c$& share(poc_dmax,doc_dmax,toc_dmax)
        DO 87 IZ=1,NZ
         DO 87 IY=1,NY
          DO 87 IX=1,NX
            POC = RPOC(IX,IY,IZ) + LPOC(IX,IY,IZ) + REPOC(IX,IY,IZ)
...
C            the PO4T equation.)
     ...
     .     + PCRB3(IX,IY,IZ)*(1.-FLOCEX)*GPP3(IX,IY,IZ)   !!! CHJT

      CDPO4T(IX,IY,IZ) = SK710 + SK89 - (1.-FPO4)*SARD
CHJT
C      WRITE(999) FRPOP*SARD,SK57,FLPOP*SARD,SK68,FRDOP*SARD,
C     .SK710,FLDOP*SARD,SK89,(1.-FPO4)*SARD,FPO4*SARD,SK8P
...
C                                         Denitrification
     ...
     .                 - SK150(IX,IY,IZ)
CHJT
C      WRITE(999) FRPON*SARD,SK1012,FLPON*SARD,SK1113,FRDON*SARD,
C     .SK1214,FLDON*SARD,SK1314,SK15P(IX,IY,IZ),(1.-FNH4)*SARD,
C     .SK1415(IX,IY,IZ),SK150(IX,IY,IZ),SR13P,SK13P
...
C            no direct source/sink terms are included in TSi equation.)
     ...
     .      + SCRB3(IX,IY,IZ)*(1.-FLOCEX)*GPP3(IX,IY,IZ)

      CDSIT(IX,IY,IZ) = SK1617 - SARD
CHJT
C      WRITE(999) SARD,SK1617,SK17P
...
C              Algal Exudate                    Oxidation
      SR22P = FLOCEX*(GPP1(IX,IY,IZ)+GPP2(IX,IY,IZ)+GPP3(IX,IY,IZ))
      SR22PA(IX,IY,IZ) = SR22P
...
C        REPOC - REACTIVE PARTICULATE ORGANIC CARBON (CSOs/STPs)
C                 Sources                         Sinks
C                   CSOs                  Hydrolysis to REDOC
      SK2324(IX,IY,IZ) = K2324CT(ISTP20(IX,IY,IZ))*REPOC(IX,IY,IZ)*TERM
      CDREPOC(IX,IY,IZ) = -SK2324(IX,IY,IZ) 
      
C        REDOC - REACTIVE DISSOLVED ORGANIC CARBON (CSOs/STPs)
C                 Sources                         Sinks
C              From RePOC                       Oxidation
      SK240(IX,IY,IZ) = K240CT(ISTP20(IX,IY,IZ))*REDOC(IX,IY,IZ)
     .            *REDOC(IX,IY,IZ)/(KMLDOC+REDOC(IX,IY,IZ))
     .            *TERM*DO(IX,IY,IZ)/(KDOC+DO(IX,IY,IZ))

      CDREDOC(IX,IY,IZ) = SK2324(IX,IY,IZ) - SK240(IX,IY,IZ)
C       BOTTLE BOD5 - INCLUDES EFFECTS OF ALGAL RESPIRATION + NITRIFICATION
     ...
     .                      + REDOC(IX,IY,IZ)*(1.-EXP(-5.*K240CT(201)))
     .                 + EXDOC(IX,IY,IZ)*(1.-EXP(-5.*K220CT(201)))
     .                 + PHYT1(IX,IY,IZ)*(1.-EXP(-5.*K1RBT(201)))
     .                 + PHYT2(IX,IY,IZ)*(1.-EXP(-5.*K2RBT(201))) 
     .                 + PHYT3(IX,IY,IZ)*(1.-EXP(-5.*K3RBT(201))) )
     .          + 4.57*DNH4(IX,IY,IZ)*(1.-EXP(-5.*K1415CT(201)))
      BOTBODHS(IX,IY,IZ) = BOTBOD(IX,IY,IZ)+
     .                 O2EQ(IX,IY,IZ)*(1.-EXP(-5.*K250CT(201)))
CHJT
C      WRITE(999) FRPOC*GRAZ(IX,IY,IZ),SK1820,FLPOC*GRAZ(IX,IY,IZ)
C     .,SK1921,FRDOC*GRAZ(IX,IY,IZ),SK200(IX,IY,IZ),FLDOC*GRAZ(IX,IY,IZ)
C     .,SK2114(IX,IY,IZ),SK210(IX,IY,IZ),SR22P,SK220(IX,IY,IZ)
C     .,SK2324(IX,IY,IZ),SK240(IX,IY,IZ)
  ...
  170 CONTINUE
      ...
      IF(IDISK.EQ.2 .OR. IDISK.EQ.3)  THEN
        DO 172 IDMP=1,NDMPS
         ...
         SK1820 = K1820CT(ISTP20(IX,IY,IZ))*RPOC(IX,IY,IZ)*TERM
         IF(IDDOPT.EQ.0)  THEN
CHJT           VAL1 = (RZ(IX,IY,IZ+1)/BVOL(IX,IY,IZ))*
CHJT     .     (PHYT1(IX,IY,IZ+1)-PHYT1(IX,IY,IZ))
CHJT           VAL2 = (RZ(IX,IY,IZ+1)/BVOL(IX,IY,IZ))*
CHJT     .     (PHYT2(IX,IY,IZ+1)-PHYT2(IX,IY,IZ))
  ...
  176   CONTINUE
     ...
     .         + EXDOC(IX,IY,IZ) 
         ...
         SR22P = FLOCEX*(GPP1(IX,IY,IZ)+GPP2(IX,IY,IZ)+GPP3(IX,IY,IZ))
         IF(IDDOPT.EQ.0)  THEN
           CALL RCAWBUF(23,REPOC(IX,IY,IZ),SK240(IX,IY,IZ),SR22P,
     .          HYDSAL(IX,IY,IZ),TOC)
         ELSE
           CALL RCAWBUF(23,REPOC_DDA(IX,IY,IZ)/IAVGDDCNTR,
     .          SK240(IX,IY,IZ),SR22P,TTDOC,
     .          TOC_DDA(IX,IY,IZ)/IAVGDDCNTR)
         ENDIF
  177   CONTINUE
        DO 178 IDMP=1,NDMPS
...
C        ALGAL PRODUCTION USING CO2 AND NO3  (HNO3 + H2O => NH4 + 2O2)
     ...
     .       + (1.-PNH43(IX,IY,IZ))*GPP3(IX,IY,IZ)
     .                   * 32.*(1./12.+2.*NCRB3(IX,IY,IZ)/14.)

C        ALGAL RESPIRATION
      SK26P = OCRB*RESP(IX,IY,IZ)
         IF(IDDOPT.EQ.0)  THEN
           CALL RCAWBUF(24,REDOC(IX,IY,IZ),SK240(IX,IY,IZ),
     .          SR26PA+SR26PB,SK26P,BOTBOD(IX,IY,IZ))
         ELSE
           CALL RCAWBUF(24,REDOC_DDA(IX,IY,IZ)/IAVGDDCNTR,
     .     SK240(IX,IY,IZ),SR26PA+SR26PB,SK26P,BOTBOD(IX,IY,IZ))
         ENDIF
  178   CONTINUE
      ENDIF

          IF (FLOOR(ITIMESECS/86400.).GE.TIMEINDEX) THEN
              TIMEINDEX = TIMEINDEX+1.0
          IF (TIMEINDEX.EQ.1.0) THEN
          READ(1034,*) ((TU(IX,IY),IX=1,NX),IY=1,NY)
          READ(1035,*) ((TV(IX,IY),IX=1,NX),IY=1,NY)
          DO 1031 IY=1,NY
          DO 1032 IX=1,NX
             TWS(IX,IY,2)=SQRT(TU(IX,IY)**2.0+TV(IX,IY)**2.0)
             TWS(IX,IY,1)=TWS(IX,IY,2)
 1031     CONTINUE
 1032     CONTINUE 
          ENDIF
          READ(1034,*) ((TU(IX,IY),IX=1,NX),IY=1,NY)
          ...
          DO 1034 IX=1,NX
              TWS(IX,IY,1)=TWS(IX,IY,2)
              TWS(IX,IY,2)=SQRT(TU(IX,IY)**2.0+TV(IX,IY)**2.0)
              TWS(IX,IY,3)= (ITIMESECS/86400.-FLOOR(ITIMESECS/86400.))
     .        *  (TWS(IX,IY,2)-TWS(IX,IY,1))+TWS(IX,IY,1)
 1034     CONTINUE
 1033     CONTINUE
      ...
      DO 1703 IY = 1,NY
      DO 1703 IX = 1,NX
      IF(FSM(IX,IY).LE.0)    GO TO 1703
C    -----------------------------------------------------------------------  by lb 20170325
C        DIC - DISSOLVED INORGANIC CARBON
C                 Sources                         Sinks
C    RDOC LDOC EXDOC REDOC Oxidation      Algal Production using
C           Algal Respiration                  NH4 and NO3    
C         LDOC Dinitrification
C            Air-Sea CO2 Flux
      TDIC1 = MAX(0.0,HYDTEMP(IX,IY,IZ))
      SDIC1 = MAX(0.0,HYDSAL(IX,IY,IZ))
C        ALGAL PRODUCTION USING CO2 AND NH4
      SR29PA = (PNH41(IX,IY,IZ)*GPP1(IX,IY,IZ)
     .                + PNH42(IX,IY,IZ)*GPP2(IX,IY,IZ)
     .                + PNH43(IX,IY,IZ)*GPP3(IX,IY,IZ))

C        ALGAL PRODUCTION USING CO2 AND NO3  (HNO3 + H2O => NH4 + 2O2)
      SR29PB = (1.-PNH41(IX,IY,IZ))*GPP1(IX,IY,IZ)
     .       + (1.-PNH42(IX,IY,IZ))*GPP2(IX,IY,IZ)
     .       + (1.-PNH43(IX,IY,IZ))*GPP3(IX,IY,IZ)

C        ALGAL RESPIRATION
C      SK26P = OCRB*RESP(IX,IY,IZ)
C    -----------------------------------------------------------------------  by lb 20170325
C        Compute surface CO2 gas exchange.
      
C     computes equilibrium partial pressure of CO2 (pCO2)    !
C              in the seawater each level    (ppmv)
C     Correction term for non-ideality, ff=k0*(1-pH2O). Equation 13 with
C     table 6 values from Weiss and Price (1980, Mar. Chem., 8, 347-359).
      ff = exp(-162.8301+218.2968/(0.01*(TDIC1+273.15))
     .   + log((0.01*(TDIC1+273.15)))*90.9241
     .   - (0.01*(TDIC1+273.15))*(0.01*(TDIC1+273.15))*1.47696
     .   + SDIC1*(0.025695-(0.01*(TDIC1+273.15))
     .   *(0.025225-(0.01*(TDIC1+273.15))*0.0049867)));
 
C     -----------------------------------------------------------------------
C     Compute first (K1) and second (K2) dissociation constant of carboinic
C     acid:

C           K1 = [H][HCO3]/[H2CO3]
C           K2 = [H][CO3]/[HCO3]

C     From Millero (1995; page 664) using Mehrbach et al. (1973) data on
C     seawater scale.
C     -----------------------------------------------------------------------

        K1=exp(2.83655-2307.1266/(TDIC1+273.15)
     .    -1.5529413*log(TDIC1+273.15)
     .    -(0.207608410+4.0484/(TDIC1+273.15))*sqrt(SDIC1)
     .    +0.0846834*SDIC1-0.00654208*SDIC1**(1.5)
     .    +log(1-0.001005*SDIC1))
        
        K2=exp(-9.226508-3351.6106/(TDIC1+273.15)
     .    -0.2005743*log(TDIC1+273.15)
     .    -(0.106901773+23.9722/(TDIC1+273.15))*sqrt(SDIC1)
     .   +0.1130822*SDIC1-0.00846934*SDIC1**(1.5)
     .   +log(1-0.001005*SDIC1))

C     -----------------------------------------------------------------------
C     Compute dissociation constant of boric acid, Kb=[H][BO2]/[HBO2].
C     From Millero (1995; page 669) using data from Dickson (1990).
C     -----------------------------------------------------------------------

        Kb=exp((-8966.9-2890.53*SDIC1**(0.5)-77.942*SDIC1
     .    +1.728*SDIC1**(1.5)-0.0996*SDIC1**2)/(TDIC1+273.15)
     .    +148.0248+137.1942*SDIC1**(0.5)+1.62142*SDIC1
     .    -(24.4344+25.085*SDIC1**(0.5)+0.2474*SDIC1)*log(TDIC1+273.15)
     .   +0.053105*SDIC1**(0.5)*(TDIC1+273.15))

C     -----------------------------------------------------------------------
C     Compute ion product of water, Kw = [H][OH].
C     From Millero (1995; page 670) using composite data.
C     -----------------------------------------------------------------------

        Kw=exp(148.9652 
     .   -(1.0/(TDIC1+273.15))*13847.26 
     .    -log(TDIC1+273.15)*23.6521 
     .    -sqrt(SDIC1)*(5.977 
     .    -(1.0/(TDIC1+273.15))*118.67 
     .    -log(TDIC1+273.15)*1.0495 )
     .    -SDIC1*0.01615 )

C     -----------------------------------------------------------------------
C     Calculate concentrations for borate (Uppstrom, 1974).
C     -----------------------------------------------------------------------

        borate=0.000232 *(SDIC1/1.80655)/10.811
C     =======================================================================

C     Solve for h in fifth-order polynomial. First calculate
C      polynomial coefficients.
        K12 = K1*K2

        p5 = -1.0
        p4 = -(TALK(IX,IY,IZ)*0.000001*1000)-Kb-K1
        p3 = (DIC(IX,IY,IZ)*0.000001*1000)*K1
     .     -(TALK(IX,IY,IZ)*0.000001*1000)
     .     *(Kb+K1)+Kb*borate+Kw-Kb*K1-K12
        p2 = (DIC(IX,IY,IZ)*0.000001*1000)*(Kb*K1+2*K12)
     .       -(TALK(IX,IY,IZ)*0.000001*1000)
     .       *(Kb*K1+K12)+Kb*borate*K1
     .       +(Kw*Kb+Kw*K1-Kb*K12)
        p1 = 2.0 *(DIC(IX,IY,IZ)*0.000001*1000)*Kb*K12
     .       -(TALK(IX,IY,IZ)*0.000001*1000)
     .       *Kb*K12+Kb*borate*K12
     .       +Kw*Kb*K1+Kw*K12
        p0 = Kw*Kb*K12;
...
C     Total Hydrogen ion concentration, Htotal = [H+].
C       X = HIC(IX,IY,IZ)
C      X0 = X
C     .    - (((((p5*X+p4)*X+p3)*X+p2)*X+p1)*X+p0)
C     .    / ((((5*p5*X+4*p4)*X+3*p3)*X+2*p2)*X+p1)
C      Htotal=X0 
C      X_lo  X_hi X_mid fni ftest
        X_lo=10.0**(-10.0)
        X_hi=10.0**(-5.0)
        X_mid=0.5*(X_lo+X_hi)
      DO Ibrack=1,30
...
C     Save pH is used again outside this routine.

C        pH(IX,IY,IZ)=-LOG10(Htotal)
      IF(ISNAN(HIC(IX,IY,IZ))) THEN
      PAUSE
      ENDIF
        
C     Add two output arguments for storing pCO2surf.

C      pCO2(IX,IY,IZ)=DIC(IX,IY,IZ)*0.000001*Htotal*Htotal/
C     .        (Htotal*Htotal+K1*Htotal+K1*K2)*1000000.0
C     P 103  71
        CO2SOL(IX,IY,IZ) = EXP(-60.2409+9345.17
     .     /  (273.15+TDIC1)
     .     +  23.3585*LOG(0.01*(273.15+TDIC1))
     .     +  SDIC1
     .     *  (0.023517-0.023656*0.01*(273.15+TDIC1)
     .     +  0.0047036*0.0001*(273.15+TDIC1)
     .     *  (273.15+TDIC1)))
        pCO2(IX,IY,IZ)=DIC(IX,IY,IZ)*Htotal*1000
     .        *Htotal/(Htotal*Htotal+K1*Htotal+K1*K2)/ff
C        Compute surface CO2 gas exchange.
      IF(IZ.EQ.1)  THEN
       SchmidtN = 1911.1-118.11*TDIC1
     .     +  3.4527*TDIC1
     .     *  TDIC1
     .     -  0.04132*TDIC1
     .     *  TDIC1*TDIC1
     .     +  SDIC1/35.*(162.-7.51*TDIC1
     .     +  0.1749*TDIC1
     .     *  TDIC1
     .     -  0.001899*TDIC1
     .     *  TDIC1
     .     *  TDIC1)
        KCCT(IX,IY) = 0.31*TWS(IX,IY,3)*TWS(IX,IY,3)
     .     *  (SchmidtN/(600.+60./35.*SDIC1))**(-0.5)
c        SR290(IX,IY,1) = KA20(IX,IY)*CO2SOL(IX,IY,IZ)
c     .     * (365.-pCO2(IX,IY,1))
        SR290(IX,IY,1) = KCCT(IX,IY)*CO2SOL(IX,IY,IZ)
     .     * (405.-pCO2(IX,IY,1))*24./100.
C    海气交换通量  Signorini_etal_2013_JGR    
      ELSE
          SR290(IX,IY,IZ) = 0.0
C          CDpCO2(IX,IY,IZ) = 0.0
      ENDIF
      
C     ALGAL RESPIRATION
C      RESP(IX,IY,IZ)
      
C      CDDIC(IX,IY,IZ) = SR290(IX,IY,IZ)/(HBAR(IX,IY)*DZ(1))
C     .    /  1000.*1800./(60.*60.*24.)
      CDDIC(IX,IY,IZ) = 1./12.*(SK200(IX,IY,IZ)+SK210(IX,IY,IZ)
     .    +  SK220(IX,IY,IZ)+SK240(IX,IY,IZ)+RESP(IX,IY,IZ)
     .    -  (SR29PA + SR29PB)
     .    +  SK2114(IX,IY,IZ))
     .    +  (-1+1.88/1.98)*2./14.*SK1415(IX,IY,IZ)
c     .    +  SR290(IX,IY,IZ)/(HBAR(IX,IY)*DZ(1))/1000.
c     .*1800./(60.*60.*24.)
      SR29PA1(IX,IY,IZ) = SR29PA
      SR29PB1(IX,IY,IZ) = SR29PB
      
C    前半部分根据碳酸盐的电离平衡计算了DIC改变对TALK的贡献
C    后半部分根据硝化反硝化的反应方程式增加了水体氢离子浓度
C      CDTALK(IX,IY,IZ) = 0.0
      CDTALK(IX,IY,IZ) = 1./12.*(SK200(IX,IY,IZ)
     .    +  SK210(IX,IY,IZ)
     .    +  SK220(IX,IY,IZ)+SK240(IX,IY,IZ)+RESP(IX,IY,IZ)
     .    -  (SR29PA + SR29PB))
     .    *  (K1*Htotal+K1*K2)
     .    /  (Htotal*Htotal+K1*Htotal+K1*K2)
     .    +  0.95*0.8*SK2114(IX,IY,IZ)/12. - 2./14.*SK1415(IX,IY,IZ)
     .    -  17./106.*SR29PA/12. + 15./106.*SR29PB/12. 
C    ----------------------------------------------------------------------- by lb 20170325    
 1703 CONTINUE
C
C             SYSTEM 25 - O2*
...
CHJT
CHJT	   write(999) SR26PA,SR26PB,SK26P   !!! ALGAL PRODUCTION AND RESPIRATION (G/M3/D)
CHJT  !!! OUTPUT FOR SYSTEM PRODUCTION AND PRESPRIATION 
         !write(999) DO(IX,IY,IZ),CS(IX,IY),SR260(IX,IY,1) !by ldh
C     .,SR26PA,SR26PB,SK26P,SK2614(IX,IY,IZ),SK26OC(IX,IY,IZ)
C     .,SK250(IX,IY,IZ)
...
CHJT             IF(EXP(-SKE(IX,IY,IZ)*DEPTH).GT.0.01) THEN
              TGPP_GDA(IX,IY,IZ) = TGPP_GDA(IX,IY,IZ) +
     .            DZ(IZ)*HBAR(IX,IY) *
     .                (GPP1(IX,IY,IZ) + GPP2(IX,IY,IZ) + GPP3(IX,IY,IZ))
...
C        FALL PHYTOPLANKTON
      ALG3FLX = ALG3VS(IX,IY,IZ)*XAZ(IX,IY)*PHYT3(IX,IY,IZ)
...
C        PARTICULATE ORGANIC MATTER
      ...
      FLUX = min(VMAXCSO,
     .        VMINCSO+(VMAXCSO-VMINCSO)*(REPOC(IX,IY,IZ)/CRCSO)**BVCSO)
     .       *VSPMTS(ISTP20(IX,IY,IZ))*XAZ(IX,IY)
      TERM = FLUX*REPOC(IX,IY,IZ)
      CDREPOC(IX,IY,IZ) = CDREPOC(IX,IY,IZ) - TERM
      CDREPOC(IX,IY,IZ+1) = CDREPOC(IX,IY,IZ+1) + TERM
  240 CONTINUE
...
CHJT      VSSED = VSNET4(IX,IY)*TCORR*XAZ(IX,IY)
     ...
     .        *VSPMTS(ISTP20(IX,IY,NZ))*VSNET4(IX,IY)*TCORR*XAZ(IX,IY)
      DEPFLUX(IX,IY,11) = VSSED*REPOC(IX,IY,NZ)
     ...
     .           *VSNET4(IX,IY)*TCORR*XAZ(IX,IY)
      ...
      DEPFLUX(IX,IY,14) = 0.0*VSSED*DIC(IX,IY,NZ)
      CDDIC(IX,IY,NZ) = CDDIC(IX,IY,NZ) - DEPFLUX(IX,IY,14)
      DEPFLUX(IX,IY,15) = 0.0*VSSED*TALK(IX,IY,NZ)
      CDTALK(IX,IY,NZ) = CDTALK(IX,IY,NZ) - DEPFLUX(IX,IY,15)
c    LB 不考虑TALK与DIC在水体中的沉降
CHJT
c	if (mod(time,1.0).eq.0) then
C	   write(999) (DEPFLUX(IX,IY,ITERM),ITERM=1,13)
C     .             ,ALGPFLX,ALGNFLX,ALGSFLX
c	end if
...
CLDH      CDPO4T(IX,IY,NZ) = CDPO4T(IX,IY,NZ)
C        NH4-N
      CDNH4T(IX,IY,NZ) = CDNH4T(IX,IY,NZ) + JNH4(IX,IY)*SA
CLDH      CDNH4T(IX,IY,NZ) = CDNH4T(IX,IY,NZ)
C        NO3-N
      CDNO23(IX,IY,NZ) = CDNO23(IX,IY,NZ) + JNO3(IX,IY)*SA
CLDH      CDNO23(IX,IY,NZ) = CDNO23(IX,IY,NZ)
C        SILICA
...
C     DIC
      CDDIC(IX,IY,NZ)=CDDIC(IX,IY,NZ)
     . +1.0*JDIC1(IX,IY)*SA
      CDDIC(IX,IY,1)=CDDIC(IX,IY,1)
     . +SR290(IX,IY,1)/1000.0*SA
C     TALK
      CDTALK(IX,IY,NZ) = CDTALK(IX,IY,NZ)
     . +1.0*JTALK1(IX,IY)*SA
CLB
c	if (mod(time,1.0).eq.0) then
C	   write(999) JPO4(IX,IY)*SA,JNH4(IX,IY)*SA,JNO3(IX,IY)*SA
C     .             ,JSI(IX,IY)*SA,SODS(IX,IY)*SA
c	end if

  260 CONTINUE

CHJT  OUTPUT FOR MASS BALANCE CALCULATION
C      WRITE(999) CDARAY
...
c$doacross local(iz,iy,ix)
     ...
     .                      + PHYT3(IX,IY,IZ) + RPOC(IX,IY,IZ)
     .                      + LPOC(IX,IY,IZ) + REPOC(IX,IY,IZ)
  325    CONTINUE
         WRITE(11)   TPOC
c$doacross local(iz,iy,ix)
         DO 330 IZ=1,NZ
          ...
          DO 345 IY=1,NY
           DO 345 IX=1,NX
             ...
             IF(EXP(-SKE(IX,IY,IZ)*DEPTH).GT.0.01) THEN
               TGPP(IX,IY,IZ) = 1000.*DZ(IZ)*DEPTH*
     .           (GPP1(IX,IY,IZ) + GPP2(IX,IY,IZ) + GPP3(IX,IY,IZ))
               TNPP(IX,IY,IZ) = 1000.*DZ(IZ)*DEPTH*
     .           (GPP1(IX,IY,IZ) + GPP2(IX,IY,IZ) + GPP3(IX,IY,IZ)
     .          - DPP1(IX,IY,IZ) - DPP2(IX,IY,IZ) - DPP3(IX,IY,IZ))
             ELSE
               TGPP(IX,IY,IZ) = 0.0
               TNPP(IX,IY,IZ) = 0.0
             ENDIF
...
C        LABELED COMMON FOR EUTROPHICATION ALGAL GROWTH ROUTINES
     ...
     .    ,KADPO4,KADSI,ISURF,ITOT,ITOTSF,KAOPT,KEOPT,KETVF
      EQUIVALENCE 
     .   (CONST(1),AGMOPT)  , (CONST(2),ACTALG)  , (CONST(3),KAOPT)   ,
     .   (CONST(4),KEOPT)   , (CONST(5),OPTION5) , (CONST(6),OPTION6)
      EQUIVALENCE 
...
C LDH          SKE(IX,IY,IZ) = KEBASE(IX,IY)
C LDH     .        + 1000.*(XKC1*CHLA1+XKC2*CHLA2+XKC3*CHLA3)
C LDH     .        + 0.052*1000000*SSC(IX,IY,IZ)                       !BY LDH
C CHANGE BY LDH ;ADD SUSPENDED SOLID AND POM IN THE EFFECT OF SHADING 
c         SKE(IX,IY,IZ)=0.052*1000000*SSC(IX,IY,IZ)
c     .+0.174*1000*(LPOC(IX,IY,IZ)
c     .+RPOC(IX,IY,IZ))+0.031*100000*(PHYT1(IX,IY,IZ)/CCHL1+
c     . PHYT2(IX,IY,IZ)/CCHL2+PHYT3(IX,IY,IZ)/CCHL3)
         SKE(IX,IY,IZ)=0.0001+0.052*1000000*SSC(IX,IY,IZ)+
     .    0.174*(LPOC(IX,IY,IZ)
     .+RPOC(IX,IY,IZ))+0.031*1000*(PHYT1(IX,IY,IZ)/CCHL1+
     . PHYT2(IX,IY,IZ)/CCHL2+PHYT3(IX,IY,IZ)/CCHL3)
C        LIGHT ATTENUATION
          IF(IZ.EQ.1)   THEN
             ITOT=ISURF
          ELSE
  ...
  105 CONTINUE
CHJT
C      WRITE(999) CARAY,BVOL     !!! OUTPUT FOR MASS BALANCE CALCULATION
...
CHJT
c	if (mod(time,1.0).eq.0) then  DPP1 (G/M3/D)
C	   write(999) RNUTR1(IX,IY,IZ),RLGHT1(IX,IY,IZ),
C     .	   GITMAX/K1C,PHYT1(IX,IY,IZ),GPP1(IX,IY,IZ),DPP1(IX,IY,IZ)
C     .       ,ITOT,HYDTEMP(IX,IY,IZ),XEMP1,XEMP2,XEMP3
C     .       ,PHYT1(IX,IY,IZ)/CCHL1*1000,TSUNRISE,TSUNSET
         !WRITE(999) TSUNRISE,TSUNSET !by ldh
c	end if
...
CHJT
c	if (mod(time,1.0).eq.0) then
C	   write(999) RNUTR2(IX,IY,IZ),RLGHT2(IX,IY,IZ),
C     .	   GITMAX/K2C,PHYT2(IX,IY,IZ),GPP2(IX,IY,IZ),DPP2(IX,IY,IZ)
C     .       ,ITOT,HYDTEMP(IX,IY,IZ),XEMP1,XEMP2,XEMP3
C     .       ,PHYT2(IX,IY,IZ)/CCHL2*1000
c	end if
...
C        COMPUTE GROWTH RATE REDUCTION DUE TO LIGHT CONDITIONS
      ...
      RLGHT3(IX,IY,IZ) = 2.718*(EXP(-TEMP2(IX,IY)*EXP(-TEMP1(IX,IY)))
     .   - EXP(-TEMP2(IX,IY)))/TEMP1(IX,IY)
...
C        COMPUTE GROWTH RATE REDUCTION DUE TO NUTRIENT LIMITATION 
C        USE 'MINIMUM' FORM RATHER THAN 'MULTIPLICATIVE' FORM 
      RNUTR3(IX,IY,IZ) = AMIN1(XEMP1,XEMP2,XEMP3)

C        EFFECT OF NUTRIENT LIMITATION ON SETTLING
      ALG3VS(IX,IY,IZ) = (VSBAS3 + VSNTR3*(1.-RNUTR3(IX,IY,IZ)))
     .   *VSBAST**STP20(IX,IY,IZ)

C        GROSS GROWTH RATE ... COMBINE FACTORS
      GPP3(IX,IY,IZ) = RNUTR3(IX,IY,IZ)*RLGHT3(IX,IY,IZ)
     .                             *GITMAX*PHYT3(IX,IY,IZ)


C        DEATH...RESPIRATION AND GRAZING

C        RESPIRATION
      RESP3 = K3RG*GPP3(IX,IY,IZ) +
     .        K3RBT(ISTP20(IX,IY,IZ))*PHYT3(IX,IY,IZ)
      RESP(IX,IY,IZ) = RESP(IX,IY,IZ) + RESP3

C        GRAZING
      GRAZ3 = K3GRZCT(ISTP20(IX,IY,IZ))*PHYT3(IX,IY,IZ) 
      GRAZ(IX,IY,IZ) = GRAZ(IX,IY,IZ) + GRAZ3

C        NET LOSS
      DPP3(IX,IY,IZ) = RESP3+GRAZ3

C        FORMULATE DERIVATIVE ... NET GROWTH (GP-DP)
      CDPHYT3(IX,IY,IZ) = ((1.-FLOCEX)*GPP3(IX,IY,IZ)-DPP3(IX,IY,IZ))
      GPDP3_DDA(IX,IY,IZ) = GPDP3_DDA(IX,IY,IZ) +
     .                          +  CDPHYT3(IX,IY,IZ)/PHYT3(IX,IY,IZ)
CHJT
c	if (mod(time,1.0).eq.0) then
C	   write(999) RNUTR3(IX,IY,IZ),RLGHT3(IX,IY,IZ),
C     .	   GITMAX/K3C,PHYT3(IX,IY,IZ),GPP3(IX,IY,IZ),DPP3(IX,IY,IZ)
C     .       ,ITOT,HYDTEMP(IX,IY,IZ),XEMP1,XEMP2,XEMP3
C     .       ,PHYT3(IX,IY,IZ)/CCHL3*1000
c	end if
...
c$doacross local(iz,iy,ix) , share(rnutr1_dda,rnutr2_dda,rnutr3_dda)
  ...
  143    CONTINUE
         ...
         IY = IFDMPS(IDMP,2)
         IZ = IFDMPS(IDMP,3)
         GPDP=(GPP3(IX,IY,IZ)-DPP3(IX,IY,IZ))/MAX(PHYT3(IX,IY,IZ),1.E-9)
         IF(IDDOPT.EQ.0)  THEN
           CALL RCAWBUF(4,PHYT3(IX,IY,IZ),1000.*PHYT3(IX,IY,IZ)/CCHL3,
     .        RLGHT3(IX,IY,IZ),RNUTR3(IX,IY,IZ),GPDP)
         ELSE
           CALL RCAWBUF(4,PHYT3_DDA(IX,IY,IZ)/IAVGDDCNTR,
     .        1000.*(PHYT3_DDA(IX,IY,IZ)/IAVGDDCNTR)/CCHL3,
...
C        STATE-VARIABLES
     ...
     .    , NO23(NX,NY,NZ)  , CDNO23(NX,NY,NZ)
     .    , SIT(NX,NY,NZ)   , CDSIT(NX,NY,NZ)
     .    , LPOC(NX,NY,NZ)  , CDLPOC(NX,NY,NZ)  !ADD BY LDH
     .    , RPOC(NX,NY,NZ)  , CDRPOC(NX,NY,NZ)    
      EQUIVALENCE
     .  (CARAY(1,1,1,1),SAL(1,1,1))   ,(CDARAY(1,1,1,1),CDSAL(1,1,1))
     . ,(CARAY(1,1,1,2),PHYT1(1,1,1)) ,(CDARAY(1,1,1,2),CDPHYT1(1,1,1))
...
C                       AND LINKAGE TO SEDIMENT FLUX SUBMODEL
     ...
     .    ,ITOT,ITOTN1,ITOTN2,ITOTN3,KADPO4,KADSI,KL
      EQUIVALENCE 
     .   (CONST(1),AGMOPT)  , (CONST(2),ACTALG)  , (CONST(3),KAOPT)   ,
     .   (CONST(4),KEOPT)   , (CONST(5),OPTION5) , (CONST(6),OPTION6)
      EQUIVALENCE 
...
C        COMPUTE KL FROM WIND SPEED

      KL = MAX(1.,0.728*SQRT(WIND) - 0.317*WIND + 0.0372*WIND*WIND)
...
C        FIXED CARBON TO CHLOROPHYLL RATIO 
          ...
          CHLA3 = PHYT3(IX,IY,IZ)/CCHL3S(IX,IY,IZ)
...
C        INCLUDE SELF-SHADING EFFECT ON EXTINCTION COEFF
C          SKE(IX,IY,IZ) = KEBASE(IX,IY)+1000.*
C     .         (XKC1*CHLA1+XKC2*CHLA2+XKC3*CHLA3)
C     .         +0.052*1000000*SSC(IX,IY,IZ)                !BY LDH
C CHANGE BY LDH ;ADD SUSPENDED SOLID AND POM IN THE EFFECT OF SHADING 
         SKE(IX,IY,IZ)=0.052*1000000*SSC(IX,IY,IZ)+0.174*(LPOC(IX,IY,IZ)
     .+RPOC(IX,IY,IZ))+0.031*1000*(PHYT1(IX,IY,IZ)/CCHL1+
     . PHYT2(IX,IY,IZ)/CCHL2+PHYT3(IX,IY,IZ)/CCHL3)
C        LIGHT ATTENUATION
          IF(IZ.EQ.1)   THEN
...
C        COMPUTE CURRENT STATE OF ALGAL STOICHIOMETRY
     ...
     .                                                 (1.+DTWQ*K3TEQ)
      ...
      RESP(IX,IY,IZ) =  RESP(IX,IY,IZ) + RESPR*PHYT3(IX,IY,IZ)

C        GROSS GROWTH RATE
      GPP3(IX,IY,IZ) = (K3C + RESPR)*PHYT3(IX,IY,IZ)
...
C        TOTAL LOSS
      DPP3(IX,IY,IZ) = RESPR*PHYT3(IX,IY,IZ) + GRAZ3
...
c$doacross local(iz,iy,ix) , share(rnutr1_dda,rnutr2_dda,rnutr3_dda)
  ...
  145   CONTINUE
     ...
     .                          MAX(PHYT3(IX,IY,IZ),1.E-9)
           CALL RCAWBUF(4,PHYT3(IX,IY,IZ),
     .        1000.*PHYT3(IX,IY,IZ)/CCHL3S(IX,IY,IZ),
