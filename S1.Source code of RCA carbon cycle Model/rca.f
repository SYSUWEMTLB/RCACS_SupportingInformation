...
C 
      open(2013,file='cpu_time.txt')                       !by ldh
      CALL DATE_AND_TIME(DMY,HMS)                          !by ldh
      WRITE(2013,3333) ' CAL BEGINS AT:','20',DMY(3:4),'-',DMY(5:6),'-',
     .             DMY(7:8),HMS(1:2),':',HMS(3:4),':',HMS(5:6)
 3333 FORMAT(5X,A,5X,6A,5X,5A)                              !by ldh
     
      OPEN(UNIT=10,FILE='RCAF10',FORM='UNFORMATTED')
      OPEN(UNIT=11,FILE=TRIM('RCAF11')//'_'//TRIM(ADJUSTL('1'))
     .      ,FORM='UNFORMATTED')
C LB 20170325
      OPEN(UNIT=524,FILE=TRIM('RCALB11')//'_'//TRIM(ADJUSTL('1'))
     .      ,FORM='UNFORMATTED')
      OPEN(UNIT=525,FILE=TRIM('SEDLB11')//'_'//TRIM(ADJUSTL('1'))
     .      ,FORM='UNFORMATTED')
      OPEN(UNIT=105,FILE=TRIM('DOCLB11')//'_'//TRIM(ADJUSTL('1'))
     .      ,FORM='UNFORMATTED')
      OPEN(UNIT=15,FILE='RCAFIC',FORM='UNFORMATTED')

C  Open files for sediment dumps and initial conditions

      OPEN(UNIT=14,FILE=TRIM('RCAF14')//'_'//TRIM(ADJUSTL('1'))
     .      ,FORM='UNFORMATTED')
...
C  Open and close/delete "mass added" file
      ...
      OPEN(1034,FILE='TVWFU.input',FORM='FORMATTED',STATUS='OLD')
      OPEN(1035,FILE='TVWFV.input',FORM='FORMATTED',STATUS='OLD')
C      OPEN(1990,FILE='INSTANT-TEMP',FORM='FORMATTED')   
C 
...
C     Call input routines
      ...
      CALL RCA03    
      CALL RCA04
C LDH ADD SKIP HOURS TO START SIMULATION(设置水动力和泥沙文件中跳过的数据，以便从自己需要的时间开始模拟）      
C        IF(SKIPHOURS.GT.0.AND.ITIMESECS.EQ.O)THEN
C          DO I=1,SKIPHOURS
             CALL RCA03A
C            UPDATE BOUNDARY CONDITION IF NECESSARILY
C             IF(IBCOPT.EQ.2.OR.IBCOPT.EQ.4.AND.I/24.GE.NXBCT) CALL RCA11
C          ENDDO
C        ENDIF 
      CALL RCA05
      CALL RCA06
...
C     Close files
      ...
      CLOSE (524,STATUS='KEEP')
      CLOSE (525,STATUS='KEEP')
      CLOSE (105,STATUS='KEEP')
      CLOSE(1034,STATUS='KEEP')
      CLOSE(1035,STATUS='KEEP')

      
	WRITE(*,*) ' RCA simulation completed! '      
      CALL DATE_AND_TIME(DMY,HMS)
      WRITE(2013,3333) ' CAL ENDS AT:','20',DMY(3:4),'-',DMY(5:6),'-',   !by ldh
     .             DMY(7:8),HMS(1:2),':',HMS(3:4),':',HMS(5:6)           !by ldh
      write(2013,'(A)')' RCA simulation completed! '
      write(OUT,'(A)')' RCA simulation completed! '
      close(2013)                                                        !by ldh
