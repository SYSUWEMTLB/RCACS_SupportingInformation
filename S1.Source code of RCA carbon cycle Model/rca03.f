...
C 
      SAVE
      INCLUDE   'RCACM' 
      INTEGER    IMINX(20),IMINY(20),IMINZ(20),TOTLWATR,SKIPHOURS
      REAL
...
CLDH,JHu
      INTEGER NOSSFILNA, ISSFILE      ! ADDED FOR READING SSC FILES
	CHARACTER,ALLOCATABLE::SSFILNA(:)*120
	REAL  TIMESSC    
      COMMON SKIPHOURS
...
C        LOCATIONS OF THE 'WET' GRID
   ...
   25   CONTINUE
      ...
      READ(IN,1000) COMMENT
      READ(IN,1100) NOHYDFILNA,SKIPHOURS
 ...
 1310 FORMAT(//40X,'OPENING HYDRODYNAMIC TRANPORT FILE = ',A40/
     .         39X,'AND OPENING DIFFUSER DISCHARGE FILE = ',A40/)
      OPEN(30,FILE=HYDFILNA(1),FORM='UNFORMATTED',STATUS='OLD',ERR=900)
...
CLDH,JHu
      READ(IN,1000) COMMENT
	READ(IN,1100) NOSSFILNA
      ALLOCATE(SSFILNA(NOSSFILNA)) 
      DO ISSFILE=1,NOSSFILNA
	   READ(IN,'(A120)') SSFILNA(ISSFILE)
	END DO
	ISSFILE=1
      OPEN(520,FILE=SSFILNA(ISSFILE),FORM='UNFORMATTED',STATUS='OLD',
     .         ERR=9520)  
...
CLDH,JHu
 1314 READ(520,END=941) TIMESSC
      READ(520) SSC      
	GO TO 30

  941 IF(NOSSFILNA.GT.1) THEN
        CLOSE(520)
        ISSFILE=ISSFILE+1
        IF(ISSFILE.GT.NOSSFILNA) THEN
           GO TO 9410
        ELSE
           WRITE(OUT,349) SSFILNA(ISSFILE)
  349      FORMAT(//5X,'OPEN FILE = ',A120,5X//)      
           OPEN(520,FILE=SSFILNA(ISSFILE),FORM='UNFORMATTED',
     .              STATUS='OLD',ERR=9520)
           GO TO 1314
         END IF
	END IF 
...
C        READ FULL LAND/WATER GRID
        ...
        READ(30)  ETA
        READ(30)  DETA
CHJT        IF(NZ.GT.1) READ(30)  HYDSAL
CHJT        IF(NZ.GT.1) READ(30)  HYDTEMP
        READ(30)  HYDSAL
        READ(30)  HYDTEMP
      ELSE
   ...
   65   CONTINUE
        ...
        IF(FSM(IX,IY).GE.1. OR. FSM(IX,IY).LE.-2)THEN
          IF(FSM(IX-1,IY).GE.1. OR. FSM(IX-1,IY).LE.-2)THEN
            AVEH = (H(IX-1,IY)+ETA(IX-1,IY)+HYDDT2*DETA(IX-1,IY)*86400.
     .           + H(IX,IY)+ETA(IX,IY)+HYDDT2*DETA(IX,IY)*86400.)/2.
            XAX(IX,IY,IZ) = AVEH*DZ(IZ)*(DY(IX-1,IY)+DY(IX,IY))/2.
          ENDIF

          IF(FSM(IX,IY-1).GE.1. OR. FSM(IX,IY-1).LE.-2)THEN
            AVEH = (H(IX,IY-1)+ETA(IX,IY-1)+HYDDT2*DETA(IX,IY-1)*86400.
     .           + H(IX,IY)+ETA(IX,IY)+HYDDT2*DETA(IX,IY)*86400.)/2.
...
C        COMPUTE AVERAGE DEPTHS FOR THE TIME-INTERVAL
        ...
        DO 75 IY=1,NY
        DO 75 IX=1,NX
          HBAR(IX,IY) = H(IX,IY)+ETA(IX,IY)+HYDDT2*DETA(IX,IY)*86400.
   75   CONTINUE
      ENDIF
...
C        (M^3/SEC * 86400. ==> M^3/DAY)
      ...
      DO 100 IY=1,NY
      DO 100 IX=1,NX
        VDER(IX,IY,IZ) = 86400.*DZ(IZ)*DETA(IX,IY)*XAZ(IX,IY)
  100 CONTINUE
      ENDIF
...
C        CONVERT FLOWS AND EXCHANGES TO M^3/DAY
  ...
  110 DO 120 IZ=1,NZ
      DO 120 IY=1,NY
      DO 120 IX=1,NX
          QX(IX,IY,IZ) = 86400.*QX(IX,IY,IZ)
          QY(IX,IY,IZ) = 86400.*QY(IX,IY,IZ)
          IF (NZ.GT.1) QZ(IX,IY,IZ) = 86400.*QZ(IX,IY,IZ)
          RX(IX,IY,IZ) = 86400.*RX(IX,IY,IZ)
          RY(IX,IY,IZ) = 86400.*RY(IX,IY,IZ)
          IF (NZ.GT.1) RZ(IX,IY,IZ) = 86400.*RZ(IX,IY,IZ)
          IF (NZ.GT.1.AND.IZ.EQ.1) RZ(IX,IY,IZ) = 0.
          EX(IX,IY,IZ) = 86400.*EX(IX,IY,IZ)
          EY(IX,IY,IZ) = 86400.*EY(IX,IY,IZ)
          IF (NZ.GT.1) EZ(IX,IY,IZ) = 86400.*EZ(IX,IY,IZ)
...
C          ERROR MESSAGE AND EXIT
 ...
 9000 FORMAT(///5X,'ERROR...THE FOLLOWING HYDRODYNAMIC FILE DOES NOT EXI
     .ST OR IS EMPTY'/5X,A40/5X,'RCA TERMINATED')
      CALL EXIT
CLDH,JHu
 9520 WRITE(OUT,95200) SSFILNA(ISSFILE)
95200 FORMAT(///5X,'ERROR...THE FOLLOWING SSC FILE DOES NOT EXI
     .ST OR IS EMPTY'/5X,A120/5X,'RCA TERMINATED')
      CALL EXIT
...
C        ERROR MESSAGE AND EXIT
 ...
 9700 FORMAT(///
     ...
     .  5X,' ... RCA TERMINATED')
CLDH,JHu
9410  IF(INPCHCK.EQ.1) RETURN
      WRITE(OUT,94100)
94100 FORMAT(///
     .  5X,'ERROR...END-OF-FILE ENCOUNTERED IN LAST SSC FILE'/
     .  5X,'NO MORE USER-SPECIFIED SSC FILES EXIST'/
     .  5X,' ... RCA TERMINATED')
      CALL EXIT
