...
%
% fillsymbols = 0 does not fill the markers but colours them
% fillsymbols ~=0 fills the markers (>0 no edge color)
%                                   (<0 grey edges)
...
if fillsymbols>0
  for i=1:length(han)
    ...
    set(han(i),'MarkerEdgeColor',0*[1 1 1])
  end
elseif fillsymbols<0
  for i=1:length(han)
    set(han(i),'MarkerFaceColor',get(han(i),'color'));
    set(han(i),'MarkerEdgeColor',0*[1 1 1])
  end
