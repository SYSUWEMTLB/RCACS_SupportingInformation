...
colorfun         = 'jet';
height           = min(SCS(3:4))*3/5;
width            = min(SCS(3:4))*3/5;
figcolor         = 'w';
TextColor        = 'k';
label.N          = 'N';
label.S          = 'S';
...
id = find(direction<0 | direction>360);
if ~isempty(id)
    error('dirction must be >0 and <360')
end
...
%% Title and Legend
title(TitleString,'color',TextColor,'fontweight',titlefontweight,'FontSize',20);         % Display a title
