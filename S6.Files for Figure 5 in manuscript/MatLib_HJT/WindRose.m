function [figure_handle,count,speeds,directions,Table] = WindRose(direction,speed,varargin)
    ...
    % 'ndirections'      Numeric.      8                     Number of direction bins (subdivisions) to be shown.
    ...
    % 'legendvariable'   String.       'WDS'                 Variable abbreviation that appears inside the legend. You can use TeX sequences.
    % 'anglenorth'       Numeric.       0                    Direction angle meaning North wind. Default is 0 for North (trigonometric reference). If you specify 'north' angle, you need to specify 'east' angle, so the script knows how angles are referenced.
    % 'angleeast'        Numeric.       90                   Direction angle meaning East wind.  Default is 90 for East (clockwise).        If you specify 'east' angle, you need to specify 'north' angle, so the script knows how angles are referenced.
    ...
    % 'legendtype'       Numeric        1                    Legend type must be continuous = 1 when figType = 2, separated boxes = 2 when figType = 1
    % 'figType'          Numeric        2                    Figure type. 1 for frequency of wind interval in each wind direction; 2 for mean speed in each wind direction 
    % 'nAveSpeed'        Numeric        5                    Only applied when figType = 2
    % 'freqroundtype'    Numeric        2                    The max round is 100% for freqroundtype=1,or the max frequency of wind for freqroundtype=2
    ...
    % 修改内容：                                                          
    %     1. 增加了figType选项：                                          
    %        figType=1则画风速频率图，figType=2则画平均风速图，默认为2 
    %     2. 增加了freqroundtype选项：
    %        freqroundtype=1，则最外圈为100%；freqroundtype=2,则最外圈是该风
    %        场数据的最大频率
    %     3. 增加了各风向的平均风速的输出                                        
    %     4. 增加了NE,NW,SE,SW的显示，将原图分为8份                  
    %
    % *********************************************************************
    %
    %% Check funciton call
...
for i=1:2:numel(varargin)
    switch lower(varargin{i})
        ...
        case 'figtype'
            FigureType       = varargin{i+1};
        case 'navespeed'
            NAveragedSpeed   = varargin{i+1}; 
        case 'freqroundtype'
            FreqRoundType    = varargin{i+1}; 
        otherwise
...
% 
if FreqRoundType~=1 & FreqRoundType~=2
    error('Errors: no such option of freqroundtype = %d',FreqRoundType)
end
...
[count AveWind] = PivotTableCount(N,n,vwinds,speed,dir,NumberElements);          % For each direction and for each speed, value of the radius that the windorose must reach (Accumulated in speed).
...
switch FigureType
    case 1
        DrawPatches1(FreqRoundType,N,n,vwinds,count,colorfun,figcolor,min_radius);% Draw the windrose, knowing the angles, the range for each direction, the speed ranges, the count (frequency) values and the colormap used.
    case 2
        DrawPatches2(FreqRoundType,N,n,count,AveWind,colorfun,figcolor,min_radius)
    otherwise
        error('Errors:No this figType, figType=%d !',FigureType)
end
...
if FreqRoundType==1
    circles   = linspace(0,100,NFrequencies+1);
    circles = circles(2:end);% Radii of the circles that must be drawn (frequencies). We do not want to spend time drawing radius=0.
    circlemax=100;
    radius    = circles   + min_radius;
    radiusmax = circlemax + min_radius;
    for iCircle=1:numel(circles)
       text(0.1,0.97*radius(iCircle),[num2str(circles(iCircle)) '%']); 
    end
else
    circles   = linspace(0,circlemax,NFrequencies+1);
    circles = circles(2:end);% Radii of the circles that must be drawn (frequencies). We do not want to spend time drawing radius=0.
    radius    = circles   + min_radius;
    radiusmax = circlemax + min_radius;
end
...
axisangles = 0:22.5:360; axisangles = axisangles(1:end-1);                 % Angles in which to draw the radial axis (trigonometric reference)
...
CardinalLabels(radiusmax,TextColor,label);                                 % Display N, S, E, W,NE,NW,SE,SW
...
elseif LegendType==1
    caxis([floor(min(AveWind)) ceil(max(AveWind))]);
    hBar=colorbar('YTick',linspace(floor(min(AveWind)),ceil(max(AveWind)),NAveragedSpeed));
    set(hBar, 'Position',[0.8900,0.2500,0.0200,0.5000]);
    text( circlemax,circlemax/sqrt(2),lablegend,'color',TextColor);
end
          
%% Outputs
[count,speeds,directions,Table] = CreateOutputs(count,vwinds,AveWind,N,n,RefN,RefE);

function [count AveWind] = PivotTableCount(N,n,vwinds,speed,dir,NumberElements)
    count  = zeros(length(N),length(vwinds));
    AveWind = zeros(length(N),1);
    for i=1:length(N)
        ...
        N_id = find(cond == 1);
        AveWind(i,1) = mean(speed(cond));
    end
    count = count/NumberElements*100;                                      % Frequency in percentage

function DrawPatches1(FreqRoundType,N,n,vwinds,count,colorfun,figcolor,min_radius)
    ...
    if FreqRoundType==1
        PropertyOpt.xlim = [-100,100]; % 100 percent
        PropertyOpt.ylim = [-100,100];
    ...
    for i=1:length(N)
        for j=length(vwinds):-1:1
            ...
            y     = [y1 r(2)*cosd(alpha)];
            fill(x,y,color(j,:),'edgecolor',hsv2rgb(rgb2hsv(color(i,:)).*[1 1 0.7])); % Draw them in the specified coloe. Edge is slightly darker.
        end
    end

function DrawPatches2(FreqRoundType,N,n,count,AveWind,colorfun,figcolor,min_radius)
    minAveWind = floor(min(AveWind));                                      % only for drawing
    maxAveWind = ceil(max(AveWind));                                       % only for drawing
    AveWindInterval=linspace(minAveWind,maxAveWind,256);
    deltaAveWind = (maxAveWind-minAveWind)/255;
    inv = strcmp(colorfun(1:3),'inv');                                     % INV = First three letters in cmap are inv
    if inv; colorfun = colorfun(4:end); end                                % if INV, cmap is the rest, excluding inv
    color = feval(colorfun,256);                                           % Create color map
    % color = interp1(linspace(minAveWind,maxAveWind,256),color,AveWindInterval);% Get the needed values.
    if inv; color = flipud(color); end;                                    % if INV, flip upside down the colormap
    plot(0,0,'.','color',figcolor,'markeredgecolor',figcolor,'markerfacecolor',figcolor); % This will create an empty legend entry.
    hold on; axis square; axis off;
    ...
    for i=1:length(N)
        ...
        x     = [x1 r(2)*sind(alpha)];                           % Create circular sectors
        y     = [y1 r(2)*cosd(alpha)];
        i_id  = intersect(find(AveWindInterval-AveWind(i)<deltaAveWind),...
            find(AveWindInterval-AveWind(i)>=0));
        if numel(i_id)==1
            fill(x,y,color(i_id,:)); % Draw them in the specified coloe. Edge is slightly darker.
        else if numel(i_id)~=0
                error('i_id should be equal to 1 ')
            end
        end
%         if FreqRoundType==2
%             text();
...
function CardinalLabels(circlemax,TextColor,labels)
    ...
    text( circlemax/sqrt(2),-circlemax/sqrt(2),[' ' labels.SE],'HorizontalAlignment','left'  ,'verticalalignment','top','color',TextColor); % Southeast  label
    text( -circlemax/sqrt(2),-circlemax/sqrt(2),[labels.SW ' '],'HorizontalAlignment','right'  ,'verticalalignment','top','color',TextColor); % Southwest  label
    text( -circlemax/sqrt(2),circlemax/sqrt(2),[labels.NW ' '],'HorizontalAlignment','right'  ,'verticalalignment','bottom','color',TextColor); % Northwest  label
    text( circlemax/sqrt(2),circlemax/sqrt(2),[' ' labels.NE],'HorizontalAlignment','left'  ,'verticalalignment','bottom','color',TextColor); % Northeast  label
...
function [count,speeds,directions,Table] = CreateOutputs(count,vwinds,AveWind,N,n,RefN,RefE)
    ...
    Table            = [{'Frequencies (%)'},{''},{'Wind Speed Interval'},repmat({''},1,numel(wspeeds)+1);'Direction Interval (?','Direction ~',wspeeds,'TOTAL','MeanSpeed';[wdirs num2cell(directions) num2cell(count) num2cell(sum(count,2)/100) num2cell(AveWind)]]; % Create table cell. Ready to xlswrite.
    Table(end+1,:)   = [{'[0 , 360)','TOTAL'},num2cell(sum(count,1)),{sum(sum(count))/100},num2cell(sum(sum(count,2).*AveWind/100))];
    Table(end+1,1:2) = {'No Direction', 'Wind Speed = 0'};  % Show Wind Speed = 0 on table.
    Table{end,end-1}   = WindZeroFreqency/100;